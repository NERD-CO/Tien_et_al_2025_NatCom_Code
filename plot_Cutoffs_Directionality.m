function plot_Cutoffs_Directionality(Settings, Directionality)
...
ticker = 1;
for dati = 1:length(Directionality)
    for alphi = 1:nalpha
        nneu = size(Directionality(dati).pkw,1);
        allps(ticker:ticker+nneu-1, :, alphi) = Directionality(dati).pkw;
    end
...
for alphi = 1:nalpha
    for cuti = 1:ncutoff
        ...
        allofem = allps(:,:,alphi) < Settings.alph2do(alphi);
        inarow = nan(nall,1);
...
plot([5*Settings.Stretch.step*1000 5*Settings.Stretch.step*1000],[0 cutsigs(cutoffs==5,1)/length(sigi)], '-', 'Color', [0.75 0.75 0.75], 'LineWidth', 1);
plot([0 5*Settings.Stretch.step*1000],[cutsigs(cutoffs==5,1)/length(sigi) cutsigs(cutoffs==5,1)/length(sigi)], '-', 'Color', [0.75 0.75 0.75], 'LineWidth', 1);
...
plot(5*Settings.Stretch.step*1000,cutsigs(cutoffs==5,1)/length(sigi), '.k', 'MarkerSize', 20);
plot(5*Settings.Stretch.step*1000,cutsigs(cutoffs==5,2)/length(sigi), '.k', 'MarkerSize', 20);
yticks([0 cutsigs(cutoffs==5,2)/length(sigi) cutsigs(cutoffs==5,1)/length(sigi) 0.5 1]);
ylim([0 1]);
